...
class TestMotus(TestPluginBase):
    ...
    def setUp(self):
        ...
        def _load_expected(fp, ncbi=False):
            df = load_motus_table(self.get_data_path(fp))
            dfs = extract_table_tax(df, ncbi=ncbi)
            return dfs 
        ...
        self.expected_single = _load_expected(os.path.join('expected', 'single-end.motus'), ncbi=True)
    ...
    def test_profile_single_end(self):
        ...
        assert_frame_equal(observed_tax, self.expected_single[1])
    
    def test_import_table(self):
        input = qiime2.Artifact.import_data(MotusMergedAbundanceTable, self.get_data_path('expected/paired-end.motus'))
        result = motus.actions.import_table(input, ncbi_taxonomy=False)
